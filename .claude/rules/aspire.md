...
- **Declare every resource here.** Postgres, cache, the API, and the Angular app are all added
  ...
  `AddNpmApp("web", "../client", "start")`). Nothing outside the AppHost invents infrastructure.
...
- **The Angular app is an `AddNpmApp` resource.** Aspire runs it and injects the API endpoint —
...
(`AddNpmApp`, client-integration methods, package names) against https://aspire.dev — these
