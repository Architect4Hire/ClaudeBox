 ...
 *   npm run e2e
 *
 * Set RECIPEBOX_WEB_URL to skip discovery and point at an already-known origin (CI, a standalone
...
function discoverWebUrl(): string {
  const override = process.env.RECIPEBOX_WEB_URL;
  if (override) return override;

  let raw: string;
  try {
    // --apphost is required: the tests run from src/RecipeBox, which contains no AppHost, and
    ...
    throw new Error(
      `Could not read Aspire state for the "${WEB_RESOURCE}" resource. Start the system with ` +
        '`aspire run`, or set RECIPEBOX_WEB_URL to target an app that is already running.',
    );
  }
